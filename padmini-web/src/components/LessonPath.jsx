...
const LessonPath = ({ onStartLesson, onStartHardPractice, lessons = [] }) => {
  const {
    completedLessonIds, userGrade, xp, streak, missedQuestions,
    userName, hearts, avatarId, dailyQuests, achievements, newAchievementNotif, clearAchievementNotif
  } = usePadminiStore();
  ...
  useEffect(() => {
    const incompleteQuest = dailyQuests?.find(q => !q.completed);
  ...
  return (
    <div className="min-h-screen bg-[#FFFEF7] pb-32 font-sinhala overflow-x-hidden relative">
        ...
        {newAchievementNotif && (
          ...
          >
            ...
            <div>
                ...
                <h4 className="font-black text-lg">{newAchievementNotif.title}</h4>
            </div>
      ...
      {/* Modern Header */}
      <div className="sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b-4 border-slate-100 p-4 shadow-sm">
        <div className="flex justify-between items-center max-w-md mx-auto">
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-1.5 bg-orange-50 px-3 py-1.5 rounded-2xl border-2 border-orange-100 shadow-sm">
                <Flame size={20} className="text-orange-500 fill-orange-500" />
                <span className="font-black text-orange-600">{streak}</span>
            </div>
            <div className="flex items-center gap-1.5 bg-rose-50 px-3 py-1.5 rounded-2xl border-2 border-rose-100 shadow-sm">
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => setShowAchievements(true)} className="bg-yellow-50 p-2 rounded-xl border border-yellow-200 text-yellow-600 hover:scale-110 transition-transform">
                <Medal size={24} />
            </button>
            <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center border-4 border-slate-100 shadow-md text-3xl">
      ...
      <div className="flex flex-col items-center max-w-md mx-auto px-6 pt-10">
        ...
        <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="w-full mb-12 flex items-end gap-4 bg-white p-6 rounded-[2.5rem] shadow-sm border-2 border-slate-50">
            ...
            <div className="flex-1 bg-blue-50 p-4 rounded-3xl rounded-bl-none relative">
                <p className="text-sm font-bold text-blue-700 italic leading-relaxed">"{buddyMessage}"</p>
        ...
        {/* Dynamic Units */}
        {units.map((unit, uIdx) => {
          const unitLessons = lessons.slice(uIdx * 4, (uIdx + 1) * 4);
          const isUnitCompleted = unitLessons.every(l => completedLessonIds.includes(l.id));
              ...
              <div className="flex flex-col items-center">
                ...
                <div className="mb-24 flex flex-col items-center gap-3">
                    <div className={`w-24 h-24 rounded-full flex items-center justify-center border-4 border-slate-100 shadow-inner
                        ...
                        <Trophy size={48} className={isUnitCompleted ? 'text-white drop-shadow-lg' : 'text-slate-300'} />
                    </div>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{unit.title} කුසලානය</p>
                </div>
              </div>
            </div>
          );
